...
export const updateOrder = catchAsync(async (req, res, next) => {
  ...
  const orderId = req.params.id; // 2. Define allowed fields for update operation.
  const allowedFields = ["status", "shippingAddress"]; // 3. Filter req.body to include only allowed fields.
  const filteredBody = {};
  ...
  if (hasUnauthorizedFields) {
    return next(new AppError("شما مجاز به تغییر همه مشخصات سفارش نیستید", 400));
  } // 5. Fetch the original order from the database before updating.
  const originalOrder = await Order.findById(orderId); // 6. Handle case where original order is not found.
  if (!originalOrder) {
    return next(new AppError("سفارشی با این شناسه یافت نشد", 404));
  } // 7. Manage product stock based on order status change // This logic specifically handles when an order is cancelled. // 7.1. Check if the 'status' field is present in the filtered body for update.
  ...
  const updatedOrder = await Order.findByIdAndUpdate(orderId, filteredBody, {
    ...
    .populate("user", "username email")
    .populate("products.product", "name price category images"); // 10. Optional: Re-check if order was found after update.
